...
class Vocabulary(object):
  ...
  def id_to_word(self, cur_id):
    ...
    raise AssertionError('Shimi DBG')
...
class CharsVocabulary(Vocabulary):
  ...
  def __init__(self, filename, max_word_length):
    ...
    valid_unicode_chars = []
    for ccc in chars_set:
      try:
        unicode_ccc = unicode(ccc)
        valid_unicode_chars.append(unicode_ccc)
      except:
        pass
    self._original_chars_set = set(valid_unicode_chars)
  ...
  def word_to_char_ids(self, word):
    ...
    # Added:
    if any(c not in self._original_chars_set for c in word):
      return self._word_char_ids[self.unk]
